...
class PackingSlips(Document):
	...
	def validate_case_nos(self):
		...
		res = frappe.db.sql("""SELECT name FROM `tabPacking Slips`
	...
	def get_details_for_packing(self):
		...
		res = frappe.db.sql("""select item_code, sum(qty) as qty,
			(select sum(psi.qty * (abs(ps.to_case_no - ps.from_case_no) + 1))
				from `tabPacking Slips` ps, `tabPacking Slips Item` psi
	...
	def get_recommended_case_no(self):
		"""
			...
			note
		"""
		recommended_case_no = frappe.db.sql("""SELECT MAX(to_case_no) FROM `tabPacking Slips`
